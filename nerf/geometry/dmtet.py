...
class DMTet:
    ...
    def map_uv(self, faces, face_gidx, max_idx):
        ...
        tex_y, tex_x = torch.meshgrid(
            torch.linspace(0, 1 - (1 / N), N, dtype=torch.float32, device=device),
            torch.linspace(0, 1 - (1 / N), N, dtype=torch.float32, device=device),
            indexing='ij'
        )
    ...
    def run(self, pos_nx3, sdf_n, tet_fx4):
        return self.__call__(pos_nx3, sdf_n, tet_fx4)
...
class DMTetGeometry(torch.nn.Module):
    ...
    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration):
        ...
        return img_loss, torch.Tensor([0.0]).cuda()#, reg_loss

###############################################################################
#  Deep Marching Tet interface
###############################################################################

class DeepMT(torch.nn.Module):
    def __init__(self, grid_res, scale=2.4):
        super(DeepMT, self).__init__()

        self.grid_res      = grid_res
        self.marching_tets = DMTet()

        tets = np.load('/home/sarahwei/code/diffrec/data/tets/{}_tets.npz'.format(self.grid_res))
        self.verts    = torch.tensor(tets['vertices'], dtype=torch.float32, device=device) * scale
        self.indices  = torch.tensor(tets['indices'], dtype=torch.long, device=device)
        # self.generate_edges()
        print("self.verts", self.verts.shape)
    ...
    def getMesh(self, sdf):
        sdf = sdf.view(-1)
        # Run DM tet to get a base mesh
        # v_deformed = self.verts + 2 / (self.grid_res * 2)
        verts, faces, uvs, uv_idx = self.marching_tets(self.verts, sdf, self.indices)
