...
def load_neu_obj(filename):
    ...
    aabb = torch.min(torch.Tensor(vertices), dim=0).values, torch.max(torch.Tensor(vertices), dim=0).values
    all_materials = [{
        'name' : '_default_neu_mat',
        'neural_tex' : None,
        'bsdf'   : 'pbr',
    }]
    ...
    if len(used_materials) > 1:
        ...
        mat = _find_mat(all_materials, "neural_tex")
        uber_material = mat
...
def load_neu_ply(filename):
    # ply_file = open(filename, 'rb')
    # load_mesh = trimesh.exchange.ply.load_ply(ply_file)
    load_mesh = trimesh.load(filename)
    ...
    vertices = torch.tensor(load_mesh.vertices, dtype=torch.float32, device='cuda')
    texcoords = None
    normals = None
    
    faces = torch.tensor(load_mesh.faces, dtype=torch.int64, device='cuda')
    tfaces = None
    nfaces = None
...
def write_ply(folder, mesh, filename=None, save_material=True):
    ply_file = os.path.join(folder, 'mesh.ply' if filename is None else filename)
    ...
    with open(ply_file, "wb") as f:
        ...
        save_mesh = trimesh.Trimesh(vertices=v_pos, faces=t_pos_idx, vertex_normals=v_nrm, face_normals=t_nrm_idx, vertex_colors=v_tex, face_colors=t_tex_idx)
        result = trimesh.exchange.ply.export_ply(save_mesh)
        f.write(result)
