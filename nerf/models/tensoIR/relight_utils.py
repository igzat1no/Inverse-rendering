...
from models.myutils import raw2alpha
import os
...
#         args=None
# ):
#     light_idx = torch.zeros((rays.shape[0], 1), dtype=torch.int32).to(device)  # [N, 1]
#     # Relight module
...
#     # imageio.imwrite('vis_light_dirs_111.png', vis_light_dirs.detach().cpu().numpy().astype(np.uint8))
#     # exit(0)
...
#     return visualize_vis, surface_xyz
...
def render_with_BRDF(
    ...
    return rgb_with_brdf, surface_xyz
...
def compute_radiance(tensoIR, surf_pts, light_in_dir, light_idx, nSample=128, vis_near=0.05, vis_far=1.5, device=None):
    ...
    app_mask = weight > tensoIR.alphaMask_thres
    if app_mask.any():
